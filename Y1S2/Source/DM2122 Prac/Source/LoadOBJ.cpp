...
)
{
	...
	while (!fileStream.eof())
	{
		...
		else if (strncmp("f ", buf, 2) == 0)
		{
			...
			{
				//cater for vertices without texture UV
	...
	float Mx = 0, My = 0, Mz = 0, mx = 0, my = 0, mz = 0;
	...
	for (unsigned i = 0; i < vertexIndices.size(); ++i)
	{
		...
		TexCoord uv;
		// check for valid value of uvIndex (>0)
		if (uvIndex > 0) uv = temp_uvs[uvIndex - 1];
		...
		if (vertex.x >= Mx) Mx = vertex.x;
		if (vertex.y >= My) My = vertex.y;
		if (vertex.z >= Mz) Mz = vertex.z;
		if (vertex.x <= mx) mx = vertex.x;
		if (vertex.y <= my) my = vertex.y;
		if (vertex.z <= mz) mz = vertex.z;
	}
	//size.Set(abs(Mx) + abs(mx), abs(My) + abs(my), abs(Mz) + abs(mz));
	size.Set(abs(mx - Mx), abs(my - My), abs(mz - Mz));
	pos.Set(mx, my, mz);
	return true;
}
