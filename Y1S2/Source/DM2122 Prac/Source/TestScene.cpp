...
void TestScene::Update(double dt)
{
	...
	if (currentCam == 0)
	{
		...
		{
			camera[0].position.x += dt * camSpeed + cos(cameraAngleX * (pi / 180));
			camera[0].position.z += dt * camSpeed + sin(cameraAngleX * (pi / 180));
			camera[0].position.y += dt * camSpeed + sin(cameraAngleY * (pi / 180));
		}
		...
		{
			camera[0].position.x += dt * camSpeed - cos((cameraAngleX + 90.f) * (pi / 180));
			camera[0].position.z += dt * camSpeed - sin((cameraAngleX + 90.f) * (pi / 180));
		}
		...
		if (Application::IsKeyPressed(VK_SPACE))	camera[0].position.y += dt * camSpeed * 100;
		if (Application::IsKeyPressed(VK_CONTROL))	camera[0].position.y -= dt * camSpeed * 100;
		...
		// Camera Logic
		camera[0].target.x = camSpeedX * cos(cameraAngleX * (pi / 180)) + camera[0].position.x;
		camera[0].target.z = camSpeedX * sin(cameraAngleX * (pi / 180)) + camera[0].position.z;
		camera[0].target.y = camSpeedY * sin(cameraAngleY * (pi / 180)) + camera[0].position.y;
	}
	...
	*/
	...
	// Shift Camera
	if (Application::IsKeyPressed('1')) currentCam = 0;
	if (Application::IsKeyPressed('2')) currentCam = 1;
	if (Application::IsKeyPressed('3')) currentCam = 2;
	if (Application::IsKeyPressed('4')) currentCam = 3;
}
