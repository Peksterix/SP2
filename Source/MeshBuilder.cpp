...
Mesh* MeshBuilder::GenerateCuboid(const std::string& meshName, Color color, float lengthX, float lengthY, float lengthZ)
{
	...
	mesh->indexSize = 36;
	mesh->mode = Mesh::DRAW_TRIANGLES;
	mesh->mSize.Set(-midvalueX, -midvalueY, -midvalueZ);
	mesh->size.Set(lengthX, lengthY, lengthZ);
...
Mesh* MeshBuilder::GenerateOBJ(const std::string& meshname, const std::string& file_path)
{
	...
	Position mSize;
	Position size;
	bool success = LoadOBJ(file_path.c_str(), vertices, uvs, normals, mSize, size);
...
Mesh* MeshBuilder::GenerateText(const std::string& meshName, unsigned numRow, unsigned numCol)
{
	...
	for (unsigned j = 0; j < numCol; ++j)
	{
		...
		{
			v.pos.Set(0, 1, 0);
			//v.texCoord.Set(width * i, 1 - (height * (j + 1)));
			v.texCoord.Set(width * i, height * (numRow - j));
			vertex_buffer_data.push_back(v);

			v.pos.Set(0, 0, 0);
			//v.texCoord.Set(width * i, 1 - (height * j));
			v.texCoord.Set(width * i, height * (numRow - j - 1));
			vertex_buffer_data.push_back(v);
			
			v.pos.Set(1, 1, 0);
			//v.texCoord.Set(width * (i + 1), 1 - (height * (j + 1)));
			v.texCoord.Set(width * (i + 1), height * (numRow - j));
			vertex_buffer_data.push_back(v);
			
			v.pos.Set(1, 0, 0);
			//v.texCoord.Set(width * (i + 1), 1 - (height * j));
