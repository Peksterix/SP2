...
void Camera::Update(double dt, Entity* obj, Position offset, Position targetOffset)
{
	...
	if (type == 0)
	{
		...
		target.x = position.x + cos(cameraAngleX * (pi / 180));
	...
	else if (type == 1)
	{
		...
		position.y = obj->position.y + offset.y * cos(obj->rotate.y * (pi / 180.0));
		position.z = obj->position.z + offset.z * cos(obj->rotate.z * (pi / 180.0));

		target.x = obj->position.x + targetOffset.x * cos(obj->rotate.x * (pi / 180.0));
		target.y = obj->position.y + targetOffset.y * cos(obj->rotate.y * (pi / 180.0));
		target.z = obj->position.z + targetOffset.z * cos(obj->rotate.z * (pi / 180.0));
	}
