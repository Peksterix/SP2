...
void PlayerName::Update(double dt)
{
	...
	if (isEditing && menuSelected[PLAYER_OPTIONS_Y] != 4)
	{
		...
		for (int i = 33; i <= 122; ++i)
			...
			{
				Application::setBounceTime(0.2f);
				tempPlayer->setName(tempPlayer->getName() + (char)i);
			}

		if (Application::IsKeyPressed(VK_BACK) && Application::getBounceTime() <= 0)
		{
			Application::setBounceTime(0.2f);
			tempPlayer->setName(tempPlayer->getName().substr(0, tempPlayer->getName().length() - 1));
		}
...
void PlayerName::Render()
{
	...
	RenderTextOnScreen(meshText, Application::getPlayer(3)->getName(), temp[3], Size[3], 3, 5, 1);
